...
图7：三组TMC2208电机驱动电路。每组驱动器配有逻辑电源`+5V`、电机电源`+12V`和独立的`100uF`电解电容，电机线圈通过ZX-XH2.54-4PZZ接插件引出，分别对应滑轨轴、俯仰/垂直轴和水平轴。

`MS1`、`MS2`和`MS3`决定细分方式，`STEP`决定步数，`DIR`决定方向，`EN`决定驱动器是否输出。TMC2208相比传统驱动器内置了更精细的电流控制，运行噪声更低，同等负载下电机发热也明显改善。逻辑电源和电机电源分开，是为了让TMC2208的控制逻辑与电机功率级分别获得合适的供电。
...
`远程指令 → ESP8266 → 灯光PWM/继电器/传感器 → TX/RX → Arduino Nano → TMC2208 → 电机 → 霍尔传感器 → Arduino Nano → TX/RX → ESP8266`。
