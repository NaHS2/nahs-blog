...
图1：灯光集成控制板实物。板上可以看到ESP8266核心板、Arduino Nano、三组带散热片的A4988电机驱动器、继电器和外部接线端子，实际装配结构与原理图中的功能分区对应。
...
电机驱动电路使用三组A4988，分别对应`SLIDER`、`TILT`和`PAN`。Arduino Nano输出使能、细分、步进和方向信号，A4988负责把这些逻辑信号转换为步进电机线圈电流。

![三组A4988电机驱动电路](../assets/light-integrated-control-board/stepper-drivers.png)

图7：三组A4988电机驱动电路。每组驱动器都有逻辑电源`+5V`、电机电源`+12V`和独立的`100uF`电解电容。

`MS1`、`MS2`和`MS3`决定细分方式，`STEP`决定步数，`DIR`决定方向，`EN`决定驱动器是否输出。逻辑电源和电机电源分开，是为了让A4988的控制逻辑与电机功率级分别获得合适的供电。

每个A4988旁边配置`100uF`电容，用于吸收电机电流变化带来的电源波动。电机电源线、驱动器地线和电解电容之间的回路应尽量短，避免电机动作时产生的干扰传入ESP8266。
