...
$$V_{AVG}=12\times\frac{5}{12}+0\times\frac{7}{12}=5V$$
...
$$D=\frac{V_{OUT}}{V_{IN}}=\frac{5}{12}\approx41.7\%$$
...
因此，前面的$D=V_{OUT}/V_{IN}$只是理想估算，实际占空比会随着输入电压、负载电流、二极管压降和芯片损耗自动调整。
...
$$I_{IN}\approx\frac{36\times1}{0.85\times12}=3.53A$$
...
$$L=\frac{(V_{IN}-V_{OUT})D}{\Delta I_L f_s}$$

一般可以把$\Delta I_L$取最大负载电流的`20%~40%`作为初始设计值。电感峰值电流为：
...
$$\Delta V_C\approx\frac{\Delta I_L}{8f_sC_{OUT}}$$
...
$$R=\frac{V_{RAIL}-V_F}{I_{LED}}$$

例如在`12V`轨上使用正向压降约`2V`、工作电流约`2mA`的LED：

$$R\approx\frac{12-2}{0.002}=5k\Omega$$
